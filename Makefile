...
.PHONY: build smoke rtl-verilog check-decode-table check-decode check-biu check-core-wait check-sleep check-bit-reg check-bit-mem check-daa-das check-adds-subs check-mulxu check-divxu check-stack-byte check-irq-vector check-trapa gnu-oracle gdb-oracle gcc-footprint isa-cases sail-coverage sail-model verify-smoke check clean
