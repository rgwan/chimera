...
object MicrocodeImage:
  ...
  val program: Map[Int, MW] = Map(
    ...
    0x7c -> MW(seq = SeqSrc.Literal, lit = Ucode.BitPrefixR16),
    ...
    Map(0x60 ->
      MW(bSel = BSel.H8, h8Idx = H8Idx.RsReg, alu = AluOp.Pass,
         wsel = WSel.Int, intIdx = IntIdx.Temp, we = true,
         seq = SeqSrc.Literal, lit = Ucode.BitRegIndex)) ++
    Map(Ucode.BitRegIndex ->
      MW(aSel = ASel.H8, h8Idx = H8Idx.RdReg, bSel = BSel.Imm8,
         intIdx = IntIdx.Temp, wsel = WSel.H8,
         seq = SeqSrc.Literal, lit = Ucode.FetchEntry))
