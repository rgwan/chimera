...
module tb_core_sleep;
  ...
  initial begin
    ...
    // vectors: NMI (6) and IRQ (8) both point at the RTE stub
    mem[16'h0006] = 8'h01; mem[16'h0007] = 8'h80;
    mem[16'h0008] = 8'h01; mem[16'h0009] = 8'h80;
    // main: set SP, hop over the vectors, then sleep/mark/mask/sleep/mark
    mem[16'h0000] = 8'h79; mem[16'h0001] = 8'h07;  // mov.w #0x0200, r7
    mem[16'h0002] = 8'h02; mem[16'h0003] = 8'h00;
    mem[16'h0004] = 8'h40; mem[16'h0005] = 8'h0a;  // bra 0x0010
    mem[16'h0010] = 8'h01; mem[16'h0011] = 8'h80;  // sleep
    mem[16'h0012] = 8'hf8; mem[16'h0013] = 8'h55;  // mov.b #0x55, r0l
    mem[16'h0014] = 8'h04; mem[16'h0015] = 8'h80;  // orc #0x80, ccr (set I)
    mem[16'h0016] = 8'h01; mem[16'h0017] = 8'h80;  // sleep
    mem[16'h0018] = 8'hf0; mem[16'h0019] = 8'haa;  // mov.b #0xaa, r0h
    ...
    reset = 0;
    // phase 1: core parks in sleep, no retire, no bus traffic
    repeat (40) @(posedge clock); #1;
    check(pc === 16'h0012 && r0[7:0] === 8'h00, "no wake: pc holds after sleep");
    req_mark = req_count;
    ...
    check({mem[16'h01fe], mem[16'h01ff]} === 16'h0012, "irq wake: stacked pc");
    repeat (40) @(posedge clock); #1;
    check(r0[7:0] === 8'h55, "irq wake: resumed after sleep");
    // phase 3: masked IRQ does not wake, NMI does
    irq = 1; repeat (2) @(posedge clock); irq = 0;
    repeat (30) @(posedge clock); #1;
    check(pc === 16'h0018 && irq_count === 1, "masked irq: still asleep");
    ...
    if (errors == 0)
      $display("CORE-SLEEP PASS: park, irq wake frame, mask, nmi wake");
    $finish;
  end
endmodule
